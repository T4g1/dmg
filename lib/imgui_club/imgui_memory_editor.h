...
struct MemoryEditor
{
    ...
    void CalcSizes(Sizes& s, size_t mem_size, size_t base_display_addr)
    {
        ...
        s.PosHexStart = (s.AddrDigitsCount + 6) * s.GlyphWidth;
    ...
    void DrawContents(void* mem_data_void_ptr, size_t mem_size, size_t base_display_addr = 0x0000)
    {
        ...
        const char* format_address = OptUpperCaseHex ? "%4.4s:%0*" _PRISizeT "X: " : " %4.4s:%0*" _PRISizeT "x: ";
        ...
        for (int line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible lines
        {
            ...
            ImGui::Text(format_address, mmu->display_address_identity(base_display_addr + addr), s.AddrDigitsCount, base_display_addr + addr);
